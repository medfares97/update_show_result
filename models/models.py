                    ...
                    for jj in current.line_ids:
                        ...
                        if jj.zone == 0 and jj.secteur > 0:
                            for hh in range(jj.secteur, jj.secteur_to + 1):
                                ...
                                params = (s2.task_id.id, s2.categ_id.id, s2.product_id.id,
                                          s2.name.replace(' - Secteur ' + str(s2.secteur), '') + ' - Secteur ' + str(
                                              hh),
                                          date_from, date_to, s2.poteau_t, poteau_t, s2.color, s2.total_t,
                                          s2.project_id.id, s2.etape, s2.gest_id.id or None, employee,
                                          s2.uom_id.id, s2.uom_id.id, s2.ftp, s2.state, s2.id, res[0] + 1,
                            ...
                            for hh in range(jj.zone, jj.zone + 1):
                                for vv in range(jj.secteur, jj.secteur_to + 1):
                                    ...
                                    params = (s2.task_id.id, s2.categ_id.id, s2.product_id.id,
                                              s2.name.replace(' - Secteur ' + str(s2.secteur), '') + ' - Zone ' + str(
                                                  hh) + ' - Secteur ' + str(vv),
                                              date_from, date_to, s2.poteau_t, poteau_t, s2.color, s2.total_t,
                                              s2.project_id.id, s2.gest_id.id or None, employee,
                                              s2.uom_id.id, s2.uom_id.id, s2.ftp, s2.state, s2.id,
                                              hh, vv, current.id, res[0] + 1, s2.etape, str(hh), is_display)
                            ...
                            for hh in range(jj.zone, jj.zone + 1):
                                ...
                                params = (s2.task_id.id, s2.categ_id.id, s2.product_id.id,
                                          ...
                                          s2.project_id.id, s2.gest_id.id or None, employee,
                                          s2.uom_id.id, s2.uom_id.id, s2.ftp, s2.state, s2.id,
                                          hh, str(jj.hh), 0, current.id, res[0] + 1, s2.etape, is_display)
